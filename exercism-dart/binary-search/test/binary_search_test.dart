...
void main() {
  ...
  test("finds a value in an array of odd length", () {
    final index =
        BinarySearch([1, 3, 5, 8, 13, 21, 34, 55, 89, 144, 233, 377, 634])
            .find(144);
  ...
  test("finds a value in an array of even length", () {
    final index =
        BinarySearch([1, 3, 5, 8, 13, 21, 34, 55, 89, 144, 233, 377]).find(21);
    expect(index, equals(5));
  }, skip: false);

  test("identifies that a value is not included in the array", () {
    expect(() => BinarySearch([1, 3, 4, 6, 8, 9, 11]).find(7),
        throwsA(isA<ValueNotFoundException>()));
  }, skip: false);

  test("a value smaller than the array's smallest value is not found", () {
    expect(() => BinarySearch([1, 3, 4, 6, 8, 9, 11]).find(0),
        throwsA(isA<ValueNotFoundException>()));
  }, skip: false);

  test("a value larger than the array's largest value is not found", () {
    expect(() => BinarySearch([1, 3, 4, 6, 8, 9, 11]).find(13),
        throwsA(isA<ValueNotFoundException>()));
  }, skip: false);

  test("nothing is found in an empty array", () {
    expect(
        () => BinarySearch([]).find(1), throwsA(isA<ValueNotFoundException>()));
  }, skip: false);

  test("nothing is found when the left and right bounds cross", () {
    expect(() => BinarySearch([1, 2]).find(0),
        throwsA(isA<ValueNotFoundException>()));
  }, skip: false);
}
